...
from keras.layers import BatchNormalization, Input, concatenate, Conv2D, add, Conv3D, Reshape, SeparableConv2D, Dropout, MaxPool2D,MaxPool3D, UpSampling2D, ZeroPadding2D, Activation, ReLU, Lambda, Dense
...
class UnetModel():
    ...
    def build_model(self):        
        ...
        thickness = Dense(10201, activation=self.params['activation'], input_shape=(1,))(thickness_beg)
        #reshape to match 101 * 101
        thickness = Reshape((101,101,1))(thickness)

        concat = concatenate([inOP,inFL, thickness],axis=-1)
