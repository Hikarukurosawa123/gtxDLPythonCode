def Model(self):
        ...
        inFL_over_RE_beg = Input(shape=(self.params['xX'],self.params['yY'],self.params['nF'], 1))
        ...
        inFL_over_RE_beg = Conv3D(filters=self.params['nFilters3D']//2, kernel_size=self.params['kernelConv3D'], strides=self.params['strideConv3D'], 
                padding='same', activation=self.params['activation'], input_shape=input_shape[1:], data_format="channels_last")(inFL_over_RE_beg)
        inFL_over_RE_beg = Dropout(0.5)(inFL_over_RE_beg)

        inFL_over_RE_beg = Conv3D(filters=int(self.params['nFilters3D']/2), kernel_size=self.params['kernelConv3D'], strides=self.params['strideConv3D'], 
                padding='same', activation=self.params['activation'], data_format="channels_last")(inFL_over_RE_beg)
        inFL_over_RE_beg = Dropout(0.5)(inFL_over_RE_beg)
        inFL_over_RE_beg = Conv3D(filters=int(self.params['nFilters3D']/2), kernel_size=self.params['kernelConv3D'], strides=self.params['strideConv3D'], 
                padding='same', activation=self.params['activation'], data_format="channels_last")(inFL_over_RE_beg)
        inFL_over_RE_beg = Dropout(0.5)(inFL_over_RE_beg)
        ...
        ## Concatenate Branch ##
        inFL_over_RE_beg = Reshape((inFL_over_RE_beg.shape[1], inFL_over_RE_beg.shape[2], inFL_over_RE_beg.shape[3] * inFL_over_RE_beg.shape[4]))(inFL_over_RE_beg)
        concat = concatenate([inFL_over_RE_beg],axis=-1)
        ...
        self.modelD = Model(inputs=[inFL_over_RE_beg], outputs=[outQF, outDF])#,outFL])
